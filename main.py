...
from commands.expressway_command import *
...
class ModerationLogView(discord.ui.View):
    ...
    def get_embed(self):
        ...
        if reason_cutted : 
            embed.set_footer(text=f"페이지 {self.page + 1}/{self.max_pages} | 일부 제재 사유는 700자를 초과하여 생략되었습니다. 제재 사유를 자세히 알아보려면 /제재내역확인 명령어에서 ID를 지정하세요.")
        else : 
...
async def handle_spamming(message, reason, timeout_d, whitelist_apply, keyword, ai_apply = False):
    ...
    log_channel = await guild.fetch_channel(get_block_log_channel(message.guild.id))
...
async def on_message_delete(message):
    ...
    if image_log : 
        ...
        log_channel = await bot.fetch_channel(image_id)
        if log_channel:
...
async def on_raw_message_edit(payload) : 
    ...
    log_channel = await bot.fetch_channel(log_id)
    if not log_channel : 
        return
...
async def on_message(message):
    ...
    if not message.author.bot : 
        ...
        if 경고_pattern:
            ...
            if len(사유2) > 1000 : 사유3 = 사유2[:1000] + " *...*"
            else : 사유3 = 사유2
            
            add_blockhistory(사용자.id, message.author.id, 사유2, "warn", 개수, message.guild.id)
            ...
            embed.add_field(name="사유", value=사유3, inline=False)
            
            channel = await bot.fetch_channel(get_block_log_channel(message.guild.id))
            if channel:
                await channel.send(embed=embed)
            ...
            add_blockhistory(사용자.id, message.author.id, 사유2, "unwarn", 개수, message.guild.id)
        ...
        if timeout_pattern:
            ...
            add_blockhistory(member.id, message.author.id, reason2, "timeout", duration, message.guild.id)
            ...
            if len(reason2) > 1000 : 사유3 = reason2[:1000] + " *...*"
            else : 사유3 = reason2
            ...
            add_blockhistory(member.id, message.author.id, reason2, "untimeout", 0, message.guild.id)
...
async def on_member_remove(member):
    ...
    async for entry in member.guild.audit_logs(limit=1, action=discord.AuditLogAction.kick):
        if entry.target.id == member.id:
            ...
            channel = await bot.fetch_channel(get_block_log_channel(guild.id))
            if channel:
                await channel.send(embed=embed)
            ...
            add_blockhistory(사용자.id, 관리자.id, 사유2, "kick", 0, guild.id)
...
async def on_member_ban(guild, user):
    async for entry in guild.audit_logs(limit=1, action=discord.AuditLogAction.ban):
        ...
        if 사유 is None :
            사유= "*(사유 입력되지 않음)*"
        사유2 = re.sub(r"\\\\n|\\n", decode_match, 사유)
        add_blockhistory(사용자.id, 관리자.id, 사유2, "ban", 0, guild.id)
        if len(사유2) > 1000 : 사유3 = 사유2[:1000] + " *...*"
        else : 사유3 = 사유2
        # Send embed to record channel
        embed = discord.Embed(
...
async def on_member_unban(guild, user):
    async for entry in guild.audit_logs(limit=1, action=discord.AuditLogAction.unban):
        if True : 
            ...
            add_blockhistory(사용자.id, 관리자.id, 사유2, "unban", 0, guild.id)
            # Send embed to record channel
            embed = discord.Embed(
                title="차단 해제",
                color = int("a5f0ff", 16),
...
async def on_member_update(before, after):
    ...
    if before.timed_out_until != after.timed_out_until:
        channel = await bot.fetch_channel(get_block_log_channel(after.guild.id))
        ...
        if before.timed_out_until and not after.timed_out_until:
            async for entry in after.guild.audit_logs(limit=1, action=discord.AuditLogAction.member_update):
                ...
                await channel.send(embed=embed)
                add_blockhistory(after.id, moderator.id, reason2, "untimeout", 0, after.guild.id)
                if after.guild.id == using_server :
                    log_channel = bot.get_channel(message_log)
                    await log_channel.send(embed=embed)
        ...
        # 타임아웃된 경우
            ...
            async for entry in after.guild.audit_logs(limit=1, action=discord.AuditLogAction.member_update):
                if entry.target.id == after.id : 
                    ...
                    add_blockhistory(after.id, moderator.id, reason2, "timeout", int(timeout_duration.total_seconds()), after.guild.id)
...
async def 경고(interaction: discord.Interaction, 사용자: discord.User, 개수: int, 사유: str):
    ...
    # 경고 추가
    result = await add_warning(interaction.guild.id, 사용자.id, 개수)
    ...
    channel = await bot.fetch_channel(get_block_log_channel(interaction.guild.id))
    if channel:
        await channel.send(embed=embed)
    ...
    add_blockhistory(사용자.id, interaction.user.id, 사유2, "warn", 개수, interaction.guild.id)
    ...
    if warn_max is not None : 
        if result[2] >= warn_max : 
            ...
            temp = await set_warning(interaction.guild.id, 사용자.id, 0)
...
async def 경고차감(interaction: discord.Interaction, 사용자: discord.User, 개수: int, 사유: str):
    ...
    # 경고 추가
    result = await remove_warning(interaction.guild.id, 사용자.id, 개수)
    ...
    add_blockhistory(사용자.id, interaction.user.id, 사유2, "unwarn", 개수, interaction.guild.id)
...
async def kick(interaction: discord.Interaction, 사용자: discord.Member, 사유: str = "None"):
    ...
    add_blockhistory(사용자.id, interaction.user.id, 사유2, "kick", 0, interaction.guild.id)
...
@app_commands.choices(
    제재내역공개여부 = [
        app_commands.Choice(name = "공개", value = "공개"), app_commands.Choice(name = "비공개", value = "비공개")
    ],
    초대링크삭제여부 = [
        app_commands.Choice(name = "예", value = "예"), app_commands.Choice(name = "아니요", value = "아니요")
    ],
)
@app_commands.describe(
    사용자 = "차단할 사용자",
    ...
    초대링크삭제여부 = "차단자가 생성한 초대 링크를 모두 삭제할지 여부"
)
@app_commands.default_permissions(ban_members=True)
async def ban(interaction: discord.Interaction, 사용자: discord.User, 사유: str = "None", 제재내역공개여부: str = "공개", 초대링크삭제여부: str = "아니요") :
    if 사용자.id == 1316579106749681664 :
        if interaction.user.id in friendly_list :
            embed = discord.Embed(
                title="오류",
    ...
    add_blockhistory(사용자.id, interaction.user.id, 사유2, "ban", 0, interaction.guild.id)
    ...
    if 초대링크삭제여부 == "예" : 
        ...
        user_invites = [invite for invite in invites if invite.inviter and invite.inviter.id == 사용자.id]
        ...
        embed = discord.Embed(
            title="완료",
            description=f"<@{사용자.id}>가 생성한 초대 링크 {deleted_count}개를 삭제했습니다.",
            color=int("a5f0ff", 16)
        )
        await interaction.followup.send(embed = embed)
...
async def bulk_ban(interaction: discord.Interaction, 사용자_리스트: str, 사유: str = "None", 제재내역공개여부: str = "공개"):
    ...
    for user_id in 사용자_id_리스트:
        try:
            ...
            add_blockhistory(user_id, interaction.user.id, 사유2, "ban", 0, interaction.guild.id)
        except Exception as e:
...
async def unban(interaction: discord.Interaction, 사용자: discord.User, 사유: str = "None", 제재내역공개여부: str = "공개") :
    ...
    add_blockhistory(사용자.id, interaction.user.id, 사유2, "unban", 0, interaction.guild.id)
...
async def bulk_unban(interaction: discord.Interaction, 사용자_리스트: str, 사유: str = "None", 제재내역공개여부: str = "공개"):
    ...
    for user_id in 사용자_id_리스트:
        try:
            user = await bot.fetch_user(int(user_id))  # 사용자 객체 가져오기
            await interaction.guild.unban(user, reason=입력사유)
            성공한_사용자.append(f"<@{user.id}>")
            add_blockhistory(user.id, interaction.user.id, 사유2, "unban", 0, interaction.guild.id)
        except Exception as e:
            실패한_사용자.append(f"<@{user_id}>")
...
async def set_log_channel(interaction: discord.Interaction, 수정삭제로그: Union[discord.TextChannel, discord.Thread] = None, 반응로그: Union[discord.TextChannel, discord.Thread] = None, 역할부여회수로그: Union[discord.TextChannel, discord.Thread] = None, 제재로그: Union[discord.TextChannel, discord.Thread] = None, 이미지로그: Union[discord.TextChannel, discord.Thread] = None):
...
async def timeout(interaction: discord.Interaction, 사용자: discord.Member, 시간: int, 단위: str = "분", 사유: str = "None", 개인응답: str = "False"):
    ...
    add_blockhistory(사용자.id, interaction.user.id, 사유2, "timeout", 시간, interaction.guild.id)
...
async def remove_timeout(interaction: discord.Interaction, 사용자: discord.Member, 사유: str = "None"):    
    ...
    add_blockhistory(사용자.id, interaction.user.id, 사유2, "untimeout", 0, interaction.guild.id)
...
async def check_moderation_log(interaction: discord.Interaction, 사용자: discord.User = None, 관리자: discord.User = None, 아이디: int = None):
    await interaction.response.defer()
    ...
    if ((사용자 is not None) or (관리자 is not None)) and (아이디 is not None) : 
        embed = discord.Embed(
            title = "오류",
            description = "`사용자`, `관리자` 옵션과 `아이디` 옵션은 동시에 사용할 수 없습니다.",
            color = discord.Color.red()
        )
        await interaction.followup.send(embed = embed)
        return
    ...
    if 사용자 and 관리자 : 
    ...
    elif 아이디 is not None : 
        c.execute(
            "SELECT * FROM blockhistory WHERE id = ? AND server_id = ? ORDER BY id DESC", 
            (아이디, interaction.guild.id)
        )
        ...
        if record : 
            ...
            id_, user_id, admin_id, reason, type_, addinfo = record[:6]
            ...
            if type_ == "timeout" :
                if addinfo is not None : 
                    if addinfo > 0 : 
                        ...
                        addinfo = str(addinfo) + "초"
            title = f"이 서버의 제재 내역 - #{id_} ({type_mapping.get(type_, '알 수 없는 제재 유형')})"
            if user_id is None : 
                user_id = "*(알 수 없음)*"
            if admin_id is None : 
                user_id = "*(알 수 없음)*"
            ...
            if len(reason) > 3000 : 
                ...
                string_io = io.StringIO(reason)
                discord_file = discord.File(fp=string_io, filename="reason.md")
...
@bot.tree.command(name="초대링크일괄삭제", description="특정 유저가 만든 초대 링크를 삭제합니다.")
@app_commands.describe(user="어느 유저가 만든 초대 링크를 삭제할지 선택")
@app_commands.default_permissions(manage_guild=True)
...
async def on_ready():
    ...
    bot.tree.add_command(expressway_command())
