...
@bot.tree.command(name="멘션지연차단", description="특정 사용자가 자신을 향해 멘션지연을 하는 것을 차단합니다.")
@app_commands.describe(사용자 = "차단할 사용자", 차단여부 = "차단 여부")
async def mention_delay_block(interaction: discord.Interaction, 사용자: discord.User, 차단여부: bool):
    await interaction.response.defer(ephemeral=True)
