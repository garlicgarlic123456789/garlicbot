...
async def 역할_정보(interaction: discord.Interaction, 역할: discord.Role):
    ...
    # 출력 형식
    if len(enabled_permissions) > 15:
        permissions = enabled_permissions[:15]
        remainder = len(enabled_permissions) - 15
...
async def list_suggestions(interaction: discord.Interaction, 페이지: int = 1):
    ...
    if author_member is None or discord.utils.get(author_member.roles, id=super_admin_id) is None:
        await interaction.response.send_message("**[오류!]** 권한이 부족합니다. 다음 권한이 필요합니다: `관리자`", ephemeral=True)
        return
...
async def 말투변경(interaction: discord.Interaction, 말투: str, 텍스트: str):
    ...
    if 말투 == "냥체" : 
        ...
        chain = create_말투변경체인(말투, "조선시대에 임금이 신하에게 말할 때(신하가 임금에게 말하는 것이 아니고 임금이 신하에게 말하는 것) 쓰는 말투입니다.")
    elif 말투 == "신하체" : 
        chain = create_말투변경체인(말투, "조선시대에 신하가 임금에게 말할 때(임금이 신하에게 말하는 것이 아니고 신하가 임금에게 말하는 것) 쓰는 말투입니다.")
    elif 말투 == "경어체" : 
