...
async def reset_chat(interaction: discord.Interaction):
    await run_reset_chat_slash_command(
        interaction,
        context={
...
async def bulk_ban(interaction: discord.Interaction, 사용자_리스트: str, 사유: str = "None", 제재내역공개여부: str = "공개"):
    await run_bulk_ban_slash_command(
        interaction,
        user_ids_text=사용자_리스트,
        reason_text=사유,
        visibility=제재내역공개여부,
        context={
            "bot": bot,
...
async def bulk_unban(interaction: discord.Interaction, 사용자_리스트: str, 사유: str = "None", 제재내역공개여부: str = "공개"):
    await run_bulk_unban_slash_command(
        interaction,
        user_ids_text=사용자_리스트,
        reason_text=사유,
        visibility=제재내역공개여부,
...
async def remove_cooldown(interaction: discord.Interaction, 사용자: discord.User):
    global error
    cooldown_result = await run_remove_summary_cooldown_slash_command(
        interaction,
        target_user=사용자,
        context={
            "developer": developer,
    ...
    error = cooldown_result.error_count
...
async def check_moderation_log(interaction: discord.Interaction, 사용자: discord.User = None, 관리자: discord.User = None):
    await run_check_moderation_log_slash_command(
        interaction,
        target_user=사용자,
        admin_user=관리자,
        context={
...
async def bulk_delete(interaction: discord.Interaction, 시작: str, 끝: str = None, 유저: discord.User = None, 사유: str = "*(사유 입력되지 않음)*"):
    await run_bulk_delete_slash_command(
        interaction,
        start_message_link=시작,
        end_message_link=끝,
        target_user=유저,
...
async def delete_invites(interaction: discord.Interaction, user: discord.User):
    await run_delete_invites_slash_command(
        interaction,
        target_user=user,
    )
...
async def 보안조치(interaction: discord.Interaction, 서버참가중단시간: str, 개인디엠중단시간: str, 사유: str = "*(사유 입력되지 않음)*") : 
    await run_security_action_slash_command(
        interaction,
        server_join_block_text=서버참가중단시간,
        dm_block_text=개인디엠중단시간,
        reason_text=사유,
    )
...
async def 제한(interaction: discord.Interaction, 유저: discord.User, 사유: str, 기간: str):
    await run_restrict_user_slash_command(
        interaction,
        target_user=유저,
        reason_text=사유,
        until_date_label=기간,
...
async def 제한해제(interaction: discord.Interaction, 유저: discord.User):
    await run_unrestrict_user_slash_command(
        interaction,
        target_user=유저,
        context={"developer": developer},
    )
...
async def 차단확인(interaction: discord.Interaction, 유저: discord.User):
    await run_check_restriction_slash_command(
        interaction,
        target_user=유저,
    )
...
async def channel_command_perm_setting(interaction: discord.Interaction, 명령어: str, 채널: discord.TextChannel, 권한: str, 역할: discord.Role = None, 유저: discord.Member = None):
    await run_channel_command_permission_slash_command(
        interaction,
        command_name=명령어,
        channel=채널,
        permission=권한,
...
async def server_command_perm_setting(interaction: discord.Interaction, 명령어: str, 권한: str, 역할: discord.Role):
    await run_server_command_permission_slash_command(
        interaction,
        command_name=명령어,
        permission=권한,
        role=역할,
...
async def 서버조언(interaction: discord.Interaction, 프롬프트: str, 메시지제공여부: bool, 채널제공여부: bool, 시작메시지: str = None, 종료메시지: str = None, 역할: str = "*(입력되지 않음)*"):
    await run_server_advice_slash_command(
        interaction,
        prompt_text=프롬프트,
        provide_messages=메시지제공여부,
        provide_channels=채널제공여부,
        start_message_link=시작메시지,
        end_message_link=종료메시지,
        role_label=역할,
        context={
            "allowed_user_ids": (
                1063676895000018944,
...
async def automod_exception_channel_setup(interaction: discord.Interaction, 채널: discord.abc.GuildChannel, 예외여부: bool):
    await run_automod_exception_channel_slash_command(
        interaction,
        channel=채널,
        enabled=예외여부,
        context={"is_blocked": is_blocked},
...
async def automod_setup(
    ...
    await run_automod_setup_slash_command(
        interaction,
        political_enabled=정치발언검열,
        sexual_enabled=성적발언검열,
        invite_enabled=초대링크검열,
        mention_enabled=멘션검열,
        whitelist_permission=검열예외권한,
        political_timeout_seconds=정치발언타임아웃기간,
        sexual_timeout_seconds=성적발언타임아웃기간,
        invite_timeout_seconds=초대링크타임아웃기간,
        mention_timeout_seconds=멘션타임아웃기간,
...
async def 격리역할설정(interaction: discord.Interaction, 격리역할: discord.Role):
    global error
    quarantine_role_result = await run_set_quarantine_role_slash_command(
        interaction,
        quarantine_role=격리역할,
        context={"is_blocked": is_blocked},
        error_count=error,
    )
    error = quarantine_role_result.error_count
...
async def 격리(interaction: discord.Interaction, 사용자: discord.User):
    global error
    quarantine_result = await run_quarantine_user_slash_command(
        interaction,
        target_user=사용자,
        context={"is_blocked": is_blocked, "using_server": using_server, "admin_role_id": admin_id},
        error_count=error,
    )
    error = quarantine_result.error_count
...
async def anti_nuke_settings(interaction: discord.Interaction, 추방차단테러: str, 로그채널: discord.TextChannel):
    global error
    anti_nuke_result = await run_anti_nuke_setting_slash_command(
        interaction,
        enabled_label=추방차단테러,
        log_channel=로그채널,
        error_count=error,
    )
    error = anti_nuke_result.error_count
...
async def anti_nuke_whitelist_settings(interaction: discord.Interaction, 유저: discord.User, 추방차단테러: str):
    global error
    anti_nuke_whitelist_result = await run_anti_nuke_whitelist_slash_command(
        interaction,
        target_user=유저,
        enabled_label=추방차단테러,
        error_count=error,
    )
    error = anti_nuke_whitelist_result.error_count
