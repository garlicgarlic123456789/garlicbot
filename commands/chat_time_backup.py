import discord    async def 채팅건수확인(self, interaction: discord.Interaction, 시작시각: str, 종료시각: str):
        if interaction.user.id != developer:rom datetime import datetime
...
def setup(bot):
    @bot.tree.command(name="채팅건수확인", description="특정 시간 동안 서버의 채팅 건수를 계산합니다.")
    @app_commands.describe(
        시작시각="시작 시각 (형식: YYYY-MM-DD HH:MM)",
        종료시각="종료 시각 (형식: YYYY-MM-DD HH:MM)"
    )
    async def 채팅건수확인(interaction: discord.Interaction, 시작시각: str, 종료시각: str):
                ...
                try:
                    async for message in channel.history(after=start_time, before=end_time, limit=None):
                        ...
                        if not message.author.bot :
                            if message.author.id not in chat_counts:
                                chat_counts[message.author.id] = 0
                            chat_counts[message.author.id] += 1
