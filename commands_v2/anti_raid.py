...
from services.database_service import update_anti_raid_settings, get_quarantine_role
...
class AntiRaidCog(commands.Cog):
    ...
    async def anti_raid_settings(
        ...
        if status:
            embed = discord.Embed(
                title="오류",
                description=f"**[오류!]** {interaction.user.id}님은 `{reason}` 사유로 {until}까지 차단 중입니다.",
                color=discord.Color.red()
            )
            await interaction.followup.send(embed=embed)
            return
