...
class SecurityCommands(commands.Cog):
    ...
    @app_commands.command(name="레이드방지설정", description="레이드 방지 기능을 설정합니다.")
    ...
    async def anti_raid_settings(
        ...
        # 비활성화인 경우
        if not 사용여부_bool:
            await update_anti_raid_settings(interaction.guild.id, 사용여부_bool, "alert", 0, 시간범위, 참가횟수)
            embed = discord.Embed(
                title="레이드 방지 설정 완료",
        ...
        if 조치방법 is None or 알림채널 is None:
            embed = discord.Embed(
                title="설정 오류",
                description="레이드 방지 기능을 활성화하려면 **조치방법**과 **알림채널**을 필수로 입력해야 합니다.",
                color=discord.Color.red()
            )
            await interaction.followup.send(embed=embed)
            return

        # 격리 역할 확인 (isolate 모드인 경우)
        if 조치방법 == "isolate" and get_quarantine_role(interaction.guild.id) is None:
            embed = discord.Embed(
                title="설정 오류",
                description="격리 역할이 설정되어 있지 않습니다.\n\n격리 역할을 먼저 설정해주세요.",
                color=discord.Color.red()
            )
            await interaction.followup.send(embed=embed)
            return
        ...
        if 시간범위 > 900 or 시간범위 < 30:
            embed = discord.Embed(
                title="설정 오류",
                description="시간범위의 값은 **30~900초** 범위에서만 설정 가능합니다.",
                color=discord.Color.red()
            )
            await interaction.followup.send(embed=embed)
            return
        ...
        if 참가횟수 > 50 or 참가횟수 < 3:
            embed = discord.Embed(
                title="설정 오류",
                description="참가횟수의 값은 **3~50회** 범위에서만 설정 가능합니다.",
                color=discord.Color.red()
            )
            await interaction.followup.send(embed=embed)
            return
        ...
        # 설정 저장
        await update_anti_raid_settings(interaction.guild.id, 사용여부_bool, 조치방법, 알림채널.id, 시간범위, 참가횟수)
        ...
        embed.add_field(
            ...
            value="• 기능 사용 여부: **활성화**\n"
                  ...
                  f"• 감지 방법: **{시간범위}초** 내에 **{참가횟수}회** 이상의 서버 참가가 발생하면 레이드로 판단",
            inline=False
        )
        ...
        self.logger.info(f"Anti-raid settings updated for {interaction.guild.name} by {interaction.user}")
